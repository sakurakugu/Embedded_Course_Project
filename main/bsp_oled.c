...
{
	oledWriteByte(0xb0 + y, OLED_CMD);//页地址模式P31,Set GDDRAM Page Start Address(PAGE0~PAGE7) for Page Addressing Mode
...
void oled_ShowNum(u8 x, u8 y, u32 num, u8 len, u8 size2)//不支持自动换行
{
	u8 t, temp;
	u8 enshow = 0;
	for (t = 0; t < len; t++)
...
void oled_ShowCHinese(u8 x, u8 y, u8 no)
{
	...
	oled_SetPos(x, y + 1);//坐标切换到下一行，显示汉字的下半部分
	for (t = 0; t < 16; t++)
	{
