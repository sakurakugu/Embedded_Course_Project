...
static void ADVANCE_TIM_GPIO_Config(void) 
{
  ...
  GPIO_InitStructure.GPIO_Pin =  ADVANCE_TIM_CH1_PIN|ADVANCE_TIM_CH2_PIN|ADVANCE_TIM_CH3_PIN;
...
/*--------------------时基结构体初始化-------------------------*/
	...
	TIM_TimeBaseStructure.TIM_Period=ADVANCE_TIM_PERIOD;	//ARR 255周期  F = TIM_CLK/{(ARR+1)*(PSC+1)}
	// 驱动CNT计数器的时钟 = Fck_int/(psc+1)
	TIM_TimeBaseStructure.TIM_Prescaler= ADVANCE_TIM_PSC;	//(SystemCoreClock / 100000 -1)	
	// 时钟分频因子 ，配置死区时间时需要用到
	TIM_TimeBaseStructure.TIM_ClockDivision=DISABLE;		
	// 计数器计数模式，设置为向上计数
	...
	// 输出通道电平极性配置,Low 当PWM信号处于有效电平时，输出低电平；无效电平时输出高电平
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low;//相当于输出占空比的信号为低电平
	// 互补输出通道电平极性配置
